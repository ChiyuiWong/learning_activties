...
def submit_quiz(quiz_id):
    ...
    try:
        ...
        is_valid_objectid = len(quiz_id) == 24 and all(c in '0123456789abcdef' for c in quiz_id.lower())
        ...
        # Return mock results for demo (for dummy IDs or when database fails)
        return jsonify({
            'success': True,
            'score': 75.0,
            'correct_answers': 1,
...
def get_quiz(quiz_id):
    ...
    try:
        ...
        # Return dummy quiz for non-ObjectId IDs or when database fails
        return jsonify({
                'id': quiz_id,
                'title': 'Sample Quiz',
