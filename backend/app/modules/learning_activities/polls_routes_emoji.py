    ...
    try:
        ...
        try:
            with get_db_connection() as client:
                ...
                print(f"[SUCCESS] 投票已保存到MongoDB: {data['question']}")
                print(f"[INFO] MongoDB ObjectId: {poll_id}")
                print(f"[INFO] 选项: {[opt for opt in data['options']]}")
            ...
            print(f"[ERROR] MongoDB连接失败: {db_error}")
...
def get_poll(poll_id):
    ...
    try:
        ...
        is_valid_objectid = len(poll_id) == 24 and all(c in '0123456789abcdef' for c in poll_id.lower())
        ...
        # Return dummy poll for non-ObjectId IDs or when database fails
        return jsonify({
                'id': poll_id,
